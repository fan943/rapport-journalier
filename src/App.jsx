...
export default function App() {
  ...
  useEffect(() => {
    try {
      localStorage.setItem(STORAGE_KEY, JSON.stringify({ date, site, etatArrivee, etatDepart, essais }));
    } catch (e) {}
  }, [date, site, etatArrivee, etatDepart, essais]);
    ...
    <div className="app">
      <style>{`
        * { box-sizing: border-box; font-family: -apple-system, system-ui, "Segoe UI", Roboto, sans-serif; }
        ...
        .etats { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
      ...
      <div className="panel">
        ...
        <div className="row">
          <label>État de la centrale</label>
          <div className="etats">
            <div>
              <label style={{ fontSize: 11 }}>À l'arrivée</label>
              <select value={etatArrivee} onChange={(e) => setEtatArrivee(e.target.value)}
                style={{ width: "100%", marginTop: 4, color: ETAT_STYLES[etatArrivee].color, fontWeight: 700 }}>
                {ETATS.map((e) => <option key={e} value={e}>{ETAT_STYLES[e].label}</option>)}
              </select>
            </div>
            <div>
              <label style={{ fontSize: 11 }}>Au départ</label>
              <select value={etatDepart} onChange={(e) => setEtatDepart(e.target.value)}
                style={{ width: "100%", marginTop: 4, color: ETAT_STYLES[etatDepart].color, fontWeight: 700 }}>
                {ETATS.map((e) => <option key={e} value={e}>{ETAT_STYLES[e].label}</option>)}
              </select>
            </div>
          </div>
